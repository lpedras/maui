	...
	where TItemsView : ItemsView
	{
		...
		protected void RemeasureLayout(VisualElement formsElement)
		{
			if (IsHorizontal)
			{
				...
				Controls.Layout.LayoutChildIntoBoundingRegion(formsElement, new Graphics.Rectangle(0, 0, request.Request.Width, CollectionView.Frame.Height));
			}
			else
			{
				var request = formsElement.Measure(CollectionView.Frame.Width, double.PositiveInfinity, MeasureFlags.IncludeMargins);
				Controls.Layout.LayoutChildIntoBoundingRegion(formsElement, new Graphics.Rectangle(0, 0, CollectionView.Frame.Width, request.Request.Height));
			}
		}
