		...
		ViewPager.IOnPageChangeListener, IManageFragments, BottomNavigationView.IOnNavigationItemSelectedListener
	{
		...
		protected override void OnLayout(bool changed, int l, int t, int r, int b)
		{
			...
			if (IsBottomTabPlacement)
			{
				...
				if (width > 0 && height > 0)
				{
					PageController.ContainerArea = new Graphics.Rectangle(0, 0, context.FromPixels(width), context.FromPixels(height - _bottomNavigationView.MeasuredHeight));
			...
			else
			{
				...
				if (width > 0 && height > 0)
				{
					PageController.ContainerArea = new Graphics.Rectangle(0, context.FromPixels(tabsHeight), context.FromPixels(width), context.FromPixels(height - tabsHeight));
